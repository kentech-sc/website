<script lang="ts">
	...
	const monthDays = $derived(schedule ? getMonthDayKeys(schedule.anchor) : []);
		...
		{#if schedule}
			<span class="month-nav">
				<button
					...
					onclick={() => nav.moveTo(addMonths(schedule.anchor, -1))}
				>
					<ChevronLeft size="1rem" />
				</button>
				<span class="month">{getMonthOfDay(schedule.anchor)}월</span>
				<button
					type="button"
					aria-label="다음 달"
					disabled={nav.loading}
